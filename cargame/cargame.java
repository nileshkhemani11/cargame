...
{
    int x=350,y=500,speed=13,cx=400,cy=650,levelx=1;
    int tr1y=430,tr2y=210,tr3y=0,tr4y=430,tr5y=210,tr6y=0,y1=0;
    int next;
    String nextlevel="First level speed less";
    int tcoin;
    int ter=0;
    int x4;
    int count=0;
    ...
    a()
    {
       ...
       spd = new JLabel("0");  
        ...
        add(collect);
        JLabel tocoin = new JLabel("Total coin");
        tocoin.setBounds(1300,410,200,40);
        tocoin.setFont(new Font("Arrial",Font.BOLD,40));
        tocoin.setForeground(Color.YELLOW);
        add(tocoin);
        totcoin = new JLabel("0/45");  
        totcoin.setFont(new Font("Arial", Font.BOLD, 40));  
        totcoin.setForeground(Color.RED);
        totcoin.setBounds(1300, 460, 130, 40);
        totcoin.setBackground(new Color(0, 0, 0));
        totcoin.setOpaque(true);
        add(totcoin);
        levely = new JLabel("Level");
        levely.setBounds(1300,510,200,40);
        levely.setFont(new Font("Arrial",Font.BOLD,40));
        levely.setForeground(Color.YELLOW);
        add(levely);
        levelz = new JLabel("1/3");  
        levelz.setFont(new Font("Arial", Font.BOLD, 40));  
        levelz.setForeground(Color.RED);
        levelz.setBounds(1300, 560, 130, 40);
        levelz.setBackground(new Color(0, 0, 0));
        levelz.setOpaque(true);
        add(levelz);
    }
    void pan()
    {
...
void fast()
{
    ...
    if(Math.abs(y - cy) < 60 && Math.abs(x - cx) < 60) {
        ...
        totcoin.setText(String.valueOf(tocoin+"/45"));
        revalidate();
        repaint();
        cx = cr();  
        cy = 0;  
    ...
    if(tcoin==15)
    {
        ...
        {
            count=11;
            nextlevel="Second Level Start Speed Increase";
            p.remove(l1);
        p.remove(coll);
        p.remove(cr2);
        ntlevel(nextlevel);
        levelz.setText(String.valueOf(levelx+"/3"));
        }
         else if(levelx==3)
        {
            levelz.revalidate();
            levelz.repaint();
            count=15;
            nextlevel="Third Level Start Speed Increase";
            p.remove(l1);
        p.remove(coll);
        p.remove(cr2);
        ntlevel(nextlevel);
...
void ntlevel(String a)
{
    changelevel = new JLabel();
changelevel.setText(a);
changelevel.setBounds(160,300,550,40);
changelevel.setFont(new Font("Arial", Font.BOLD, 33));
p.add(changelevel);
collect.setText("");
level=new JLabel();
            for(int i=5;i>0;i--)
            {
                ...
                level.setBounds(400,400,600,80);
                level.setFont(new Font("Arial", Font.BOLD, 80));
                p.revalidate();
                p.repaint();
                p.add(level);
                Thread.sleep(1000);
